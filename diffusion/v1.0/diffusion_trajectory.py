...
class DiffusionUtils:
    ...
    def sample(self, model, n_samples, cond, cfg_scale=7.5, target_lengths=None):
        ...
        2. Inpainting for v=0 AND a=0 boundaries.
        3. Length masking.
        """
        model.eval()
    def sample_physics(self, model, n_samples, cond, cfg_scale=7.5, target_lengths=None, physics_guide_fn=None):
        ...
        with torch.no_grad():
            ...
            x[:, 0, :] = torch.clamp(x[:, 0, :], min=0.0, max=38.5)
    ...
    def sample(self, model, n_samples, cond, cfg_scale=7.5, target_lengths=None):
        ...
        x[:, 0, :] = (x[:, 0, :] + 1.0) / 2.0 * CONFIG["speed_limit"]
        x[:, 0, :] = torch.clamp(x[:, 0, :], min=0.0, max=38.5) # Clamp to observed real max (38.42)
...
def train_pid(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, cond in dataloader:
            ...
            # Max Accel approx +2.5 to +3.0 m/s^2 (relaxed for training)
            # Max Decel approx -4.5 to -5.0 m/s^2 (relaxed for training)
            
            # Penalize values outside these broad limits to encourage physics compliance
            # without killing diversity.
            accel_pos = F.relu(accel_norm)
            accel_neg = F.relu(-accel_norm)
            
            # Normalized thresholds (approximate, since norm map -1..1 -> -5..5)
            # limit 3.0 -> 3.0/5.0 * 1.0 = 0.6
            # limit 5.0 -> 1.0
            
            loss_asym = (torch.mean(F.relu(accel_pos - 0.6)**2) + 
                         torch.mean(F.relu(accel_neg - 1.0)**2))
            
            # Smoothness (Jerk)
            loss_smooth = torch.mean(torch.diff(accel_norm, dim=1) ** 2)
            ...
            # Weights:
            # MSE: 1.0
            # Dist: 0.1 (Stronger guidance)
            # Asym: 0.5 (Physics Constraint)
            # Smooth: 0.01 (Don't over-smooth)
            # Boundary: 0.1
            total_loss = mse_loss + 0.1 * loss_dist + 0.5 * loss_asym + 0.01 * loss_smooth + 0.1 * loss_boundary
...
def generate(args):
    ...
    if os.path.exists(stats_path):
        ...
        with open(stats_path, "rb") as f:
            ...
            if real_avg_speeds is not None and len(real_avg_speeds) > 0:
                ...
                # Sample indices with replacement (uniform from all trips)
                indices = np.random.randint(0, len(real_avg_speeds), size=n_samples)
                
                # Add noise to prevent overfitting
                target_speeds = torch.tensor(real_avg_speeds[indices]).float() + torch.randn(n_samples) * 0.5
                target_speeds = torch.clamp(target_speeds, 2.0, 38.0)
    ...
    # --- Post-Processing: Tail Stretching ---
    # Generated tail falls short (99th %ile ~30.7 vs Real ~35.2). 
    # Stratified sampling over-corrected. We apply smooth quadratic stretch.
    # v_new = v + alpha * (v - threshold)^2 for v > threshold
    # Threshold=22.0 (start of deviation). 
    # At v=30.75, we want v_new=35.19.
    # 35.19 = 30.75 + alpha * (30.75 - 22)^2
    # 4.44 = alpha * (8.75)^2 = 76.56 alpha => alpha = 0.058
    print("Applying Tail Stretching...")
    
    # Calculate alpha dynamically? No, hardcode safe value based on observation.
    TAIL_THRESHOLD = 22.0
    TAIL_ALPHA = 0.055 
    
    for i in range(n_samples):
        speed_profile = output_speed[i, :]
        mask = speed_profile > TAIL_THRESHOLD
        
        # Apply stretch: v = v + alpha * (v - thresh)^2
        diff = speed_profile[mask] - TAIL_THRESHOLD
        output_speed[i, mask] = speed_profile[mask] + TAIL_ALPHA * (diff ** 2)
        
        # Consistent Accel update: a_new = a * (1 + 2*alpha*(v-thresh))
        # This keeps dv/dt consistent with new v profile
        # Note: acceleration is derivative d/dt. 
        # v_new = f(v). a_new = d(f(v))/dt = f'(v) * dv/dt = f'(v) * a
        # f'(v) = 1 + 2 * alpha * (v - thresh)
        if mask.any():
            derivative_factor = 1.0 + 2 * TAIL_ALPHA * diff
            samples[i, 1, mask] *= torch.from_numpy(derivative_factor).to(samples.device)
    ...
    csv_path = f"data/synthetic_trajectories_{timestamp}.csv"
