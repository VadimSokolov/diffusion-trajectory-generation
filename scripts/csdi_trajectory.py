...
CONFIG = {
    ...
    # Physics-based constraint weights (vehicle-agnostic, covers passenger cars to heavy trucks)
    # -------------------------------------------------------------------------
    # Vehicle type      | Typical max accel | Typical max decel | Notes
    # -------------------------------------------------------------------------
    # Sports car        | 4-6 m/s²          | 10-12 m/s²        | High performance
    # Passenger car     | 2.5-4 m/s²        | 8-10 m/s²         | Average
    # SUV/Pickup        | 2-3.5 m/s²        | 7-9 m/s²          | Heavier
    # Bus               | 1-2 m/s²          | 4-6 m/s²          | Passenger comfort
    # Heavy duty truck  | 0.5-1.5 m/s²      | 3-5 m/s²          | Loaded
    # -------------------------------------------------------------------------
    # Using generous thresholds to accommodate ALL vehicle types
    "accel_penalty_weight": 0.03,    # Soft penalty for extreme accelerations
    "accel_threshold": 4.0,          # Max realistic accel m/s² (covers all vehicles)
    "decel_threshold": 5.0,          # Max realistic decel m/s² (comfortable emergency braking)
    "jerk_penalty_weight": 0.02,     # Penalize sudden acceleration changes (comfort)
    "jerk_threshold": 2.0,           # Max comfortable jerk m/s³
    "accel_distribution_weight": 0.05, # Encourage narrow accel distribution (data-driven)
}
...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, data_path, max_len=CONFIG["max_length"], 
        ...
        if self.augment:
            print("Vehicle type augmentation ENABLED (generating truck, bus, sports car variants)")
        ...
        for f in tqdm(files, desc="Loading"):
            try:
                ...
                # Compute vehicle dynamics indicator (acceleration capability)
                # This captures whether trajectory is from aggressive/capable vehicle
                # vs smooth/heavy vehicle
                if len(speed) > 1:
                    accel = np.diff(speed)  # m/s² at 1Hz
                    accel_p90 = np.percentile(np.abs(accel), 90)  # 90th percentile |accel|
                    max_accel = np.max(accel)  # Max acceleration
                    max_decel = np.abs(np.min(accel))  # Max deceleration
                else:
                    accel_p90 = 0.5
                    max_accel = 0.5
                    max_decel = 0.5
                ...
                # Normalize conditions (now includes vehicle dynamics)
                # Vehicle type indicator: accel_p90 normalized by typical range
                # - Heavy truck: accel_p90 ~ 0.3-0.8 m/s² → 0.1-0.3 normalized
                # - Passenger car: accel_p90 ~ 1.0-2.5 m/s² → 0.3-0.8 normalized
                # - Sports car: accel_p90 ~ 2.0-4.0 m/s² → 0.7-1.0 normalized
                vehicle_dynamics = np.clip(accel_p90 / 3.0, 0, 1)  # Normalize by 3 m/s²
                
                cond = np.array([
                    avg_speed / 30.0,
                    duration / 1000.0,
                    max_speed / 40.0,
                    vehicle_dynamics,  # NEW: vehicle acceleration capability
                ], dtype=np.float32)
                self.conditions.append(cond)
                
                # ============================================================
                # Vehicle Type Augmentation
                # Since training data is mostly passenger cars, we augment to 
                # create synthetic examples for other vehicle types
                # ============================================================
                if self.augment and len(speed) > 20:
                    # Create heavy truck variant (smoother accelerations)
                    # Apply heavy smoothing to simulate slower acceleration dynamics
                    truck_speed = self._smooth_for_vehicle(speed, vehicle_type='heavy_truck')
                    if truck_speed is not None:
                        self._add_trajectory(truck_speed, duration, target_dynamics=0.15)
                    
                    # Create bus variant (moderately smooth)
                    bus_speed = self._smooth_for_vehicle(speed, vehicle_type='bus')
                    if bus_speed is not None:
                        self._add_trajectory(bus_speed, duration, target_dynamics=0.35)
        ...
        print(f"Loaded {len(self.data)} valid trajectories (including augmented)")
    
    def _smooth_for_vehicle(self, speed, vehicle_type='heavy_truck'):
        """
        Create a smoothed version of trajectory to simulate different vehicle types.
        Heavy trucks have slower acceleration dynamics than passenger cars.
        
        Physics-based approach:
        1. Convert to acceleration space
        2. Apply acceleration limits for target vehicle type
        3. Clip extreme accelerations
        4. Reconstruct speed from limited accelerations
        """
        if len(speed) < 10:
            return None
        
        # Vehicle-specific acceleration limits (m/s²)
        limits = {
            'heavy_truck': {'max_accel': 1.0, 'max_decel': 2.0, 'smooth_window': 7},
            'truck': {'max_accel': 1.5, 'max_decel': 2.5, 'smooth_window': 5},
            'bus': {'max_accel': 1.2, 'max_decel': 2.5, 'smooth_window': 5},
            'suv': {'max_accel': 2.5, 'max_decel': 4.0, 'smooth_window': 3},
        }
        
        params = limits.get(vehicle_type, limits['heavy_truck'])
        
        # Get accelerations
        accel = np.diff(speed)
        
        # Clip to vehicle-specific limits
        accel_clipped = np.clip(accel, -params['max_decel'], params['max_accel'])
        
        # Apply smoothing to reduce rapid changes
        window = params['smooth_window']
        if window > 1 and len(accel_clipped) > window:
            kernel = np.ones(window) / window
            accel_smooth = np.convolve(accel_clipped, kernel, mode='same')
            # Re-clip after smoothing
            accel_smooth = np.clip(accel_smooth, -params['max_decel'], params['max_accel'])
        else:
            accel_smooth = accel_clipped
        
        # Reconstruct speed from accelerations
        new_speed = np.zeros_like(speed)
        new_speed[0] = speed[0]  # Keep starting speed
        new_speed[1:] = np.cumsum(accel_smooth) + speed[0]
        
        # Ensure non-negative and proper ending
        new_speed = np.clip(new_speed, 0, CONFIG["speed_limit"])
        
        # Force end at 0 (smooth ramp down in last 10% of trajectory)
        ramp_len = max(5, len(new_speed) // 10)
        if new_speed[-1] > 0.5:  # If not already near zero
            ramp = np.linspace(1, 0, ramp_len)
            new_speed[-ramp_len:] *= ramp
        
        return new_speed.astype(np.float32)
    
    def _add_trajectory(self, speed, duration, target_dynamics):
        """Add an augmented trajectory to the dataset."""
        avg_speed = np.mean(speed)
        max_speed = np.max(speed)
        ...
        if duration > self.max_len:
            speed_pad = speed_norm[:self.max_len]
        else:
            speed_pad = np.pad(speed_norm, (0, self.max_len - duration), 
                              mode='constant', constant_values=-1)
            mask[duration:] = 0
        ...
        # Use target vehicle dynamics value
        cond = np.array([
            avg_speed / 30.0,
            duration / 1000.0,
            max_speed / 40.0,
            target_dynamics,  # Explicit target for this vehicle type
        ], dtype=np.float32)
        self.conditions.append(cond)
...
class CSDITransformer(nn.Module):
    ...
    def __init__(self, d_model=256, n_heads=8, n_layers=4, d_ff=512,
                 dropout=0.1, max_len=512, cond_dim=4, num_diffusion_steps=100):
        super().__init__()
...
class CSDISampler:
    ...
    def sample(self, cond, lengths, n_steps=None, boundary_ramp=3, smooth_kernel=9, 
               correlated_noise_std=0.0, noise_correlation_length=10):
        """
        ...
        Args:
            ...
            correlated_noise_std: std of correlated noise to add (for acceleration tails)
            noise_correlation_length: correlation length for noise smoothing
        ...
        # Add correlated noise to restore natural acceleration variation
        if correlated_noise_std > 0:
            x = self._add_correlated_noise(x, lengths, noise_std=correlated_noise_std, 
                                           correlation_length=noise_correlation_length)
            print(f"    Correlated noise added (std={correlated_noise_std}, corr_len={noise_correlation_length})")
    ...
    def _add_correlated_noise(self, x, lengths, noise_std=0.05, correlation_length=10):
        """
        Add correlated (smooth) noise to restore natural acceleration variation.
        
        This adds medium-frequency variation without high-frequency jitter,
        helping match the acceleration distribution tails of real data.
        
        Args:
            x: (B, L) trajectories in normalized [-1, 1] space
            lengths: (B,) valid lengths
            noise_std: standard deviation of noise (in normalized space)
            correlation_length: smoothing kernel for noise (higher = smoother noise)
        
        Returns:
            x with correlated noise added
        """
        B, L = x.shape
        
        # Create smoothing kernel for noise
        sigma = correlation_length / 4.0
        kernel_size = correlation_length * 2 + 1
        kernel = torch.exp(-torch.linspace(-correlation_length, correlation_length, 
                                           kernel_size, device=x.device)**2 / (2*sigma**2))
        kernel = kernel / kernel.sum()
        kernel = kernel.view(1, 1, -1)
        ...
        for i in range(B):
            ...
            noise = torch.randn(length, device=x.device) * noise_std
            
            # Smooth the noise to make it correlated (low-frequency)
            noise_padded = F.pad(noise.unsqueeze(0).unsqueeze(0), 
                                 (correlation_length, correlation_length), mode='replicate')
            smooth_noise = F.conv1d(noise_padded, kernel).squeeze()
            
            # Scale to desired std after smoothing
            if smooth_noise.std() > 0:
                smooth_noise = smooth_noise * (noise_std / smooth_noise.std())
            
            # Add to trajectory (avoid boundaries - will be handled by ramps)
            margin = max(5, correlation_length)
            if length > 2 * margin:
                x[i, margin:length-margin] += smooth_noise[margin:length-margin]
            
            # Clip to valid range
            x[i, :length] = torch.clamp(x[i, :length], -1, 1)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, cond, mask in pbar:
            ...
            # ============================================================
            # Physics-based losses (inspired by Markov Chain approach)
            # ============================================================
            
            # Estimate denoised trajectory from noise prediction
            # x_denoised ≈ (x_noisy - sqrt(1-alpha_bar) * noise_pred) / sqrt(alpha_bar)
            alpha_bar = schedule.alpha_bars[t].view(-1, 1)
            x_denoised = (x_noisy - torch.sqrt(1 - alpha_bar) * noise_pred) / torch.sqrt(alpha_bar).clamp(min=0.01)
            x_denoised = torch.clamp(x_denoised, -1, 1)  # Keep in valid range
            
            # Compute acceleration from denoised trajectory (in normalized space)
            # Note: In normalized [-1,1] space, acceleration is diff of speed
            accel = x_denoised[:, 1:] - x_denoised[:, :-1]  # (B, L-1)
            
            # 1. Asymmetric acceleration/deceleration penalty
            # Real driving: vehicles accelerate slower than they brake
            accel_threshold_norm = CONFIG["accel_threshold"] / CONFIG["speed_limit"] * 2
            decel_threshold_norm = CONFIG["decel_threshold"] / CONFIG["speed_limit"] * 2
            
            # Separate penalties for acceleration (positive) and deceleration (negative)
            accel_positive = F.relu(accel)  # Only positive accelerations
            accel_negative = F.relu(-accel)  # Only negative (deceleration magnitude)
            
            accel_excess = F.relu(accel_positive - accel_threshold_norm)
            decel_excess = F.relu(accel_negative - decel_threshold_norm)
            
            accel_penalty = ((accel_excess ** 2 + decel_excess ** 2) * diff_mask).sum() / diff_mask.sum().clamp(min=1)
            
            # 2. Jerk penalty (penalize large changes in acceleration)
            # jerk = d(accel)/dt = accel[t+1] - accel[t]
            # Comfortable jerk typically < 1-2 m/s³ for passenger comfort
            jerk = accel[:, 1:] - accel[:, :-1]  # (B, L-2)
            jerk_mask = diff_mask[:, 1:] * diff_mask[:, :-1]
            jerk_threshold_norm = CONFIG.get("jerk_threshold", 2.0) / CONFIG["speed_limit"] * 2
            jerk_excess = F.relu(torch.abs(jerk) - jerk_threshold_norm)
            jerk_penalty = (jerk_excess ** 2 * jerk_mask).sum() / jerk_mask.sum().clamp(min=1)
            
            # 3. Acceleration distribution loss (encourage concentration around 0)
            # Penalize variance of acceleration - want narrow distribution like real data
            accel_valid = accel * diff_mask
            accel_var = (accel_valid ** 2).sum() / diff_mask.sum().clamp(min=1)
            # Target variance based on real data: ~0.6 m/s² std → 0.36 variance
            # In normalized space: 0.6/40*2 ≈ 0.03, variance ≈ 0.0009
            target_var_norm = (0.6 / CONFIG["speed_limit"] * 2) ** 2
            accel_dist_loss = F.relu(accel_var - target_var_norm)  # Only penalize if too wide
            
            # Combined loss
            smoothness_weight = CONFIG.get("smoothness_weight", 0.1)
            accel_weight = CONFIG.get("accel_penalty_weight", 0.05)
            jerk_weight = CONFIG.get("jerk_penalty_weight", 0.02)
            accel_dist_weight = CONFIG.get("accel_distribution_weight", 0.1)
            
            loss = (mse_loss + 
                    smoothness_weight * smoothness_loss +
                    accel_weight * accel_penalty +
                    jerk_weight * jerk_penalty +
                    accel_dist_weight * accel_dist_loss)
...
def generate(args):
    ...
    for f in files[:min(len(files), 2000)]:  # Sample from subset for speed
        try:
            ...
            if "speedMps" in df.columns:
                ...
                if len(speed) >= 50:
                    # Compute vehicle dynamics from acceleration
                    accel = np.diff(speed) if len(speed) > 1 else [0.5]
                    accel_p90 = np.percentile(np.abs(accel), 90)
                    vehicle_dynamics = np.clip(accel_p90 / 3.0, 0, 1)
    ...
    if len(real_conditions) > 0:
        ...
        # Weighted sampling to boost high-speed trajectories
        # This helps match the right tail of the speed distribution
        all_avg_speeds = np.array([c['avg_speed'] for c in real_conditions])
        
        # Compute weights: boost probability for high-speed trips
        # Using speed^power where power > 1 boosts high speeds
        speed_boost_power = getattr(args, 'speed_boost', 1.5)  # 1.0 = uniform, 2.0 = strong boost
        weights = np.power(all_avg_speeds + 1, speed_boost_power)  # +1 to avoid zero
        weights = weights / weights.sum()  # Normalize to probability
        
        # Sample with weighted replacement
        sampled_idx = np.random.choice(len(real_conditions), n_samples, replace=True, p=weights)
        sampled = [real_conditions[i] for i in sampled_idx]
        
        # Log the effect
        original_mean = np.mean(all_avg_speeds)
        sampled_mean = np.mean([c['avg_speed'] for c in sampled])
        ...
        # Vehicle dynamics: use from data OR override with command-line arg
        if hasattr(args, 'vehicle_type') and args.vehicle_type is not None:
            # Override with specified vehicle type
            # 0.0-0.3 = heavy truck, 0.3-0.6 = bus/suv, 0.6-0.8 = passenger car, 0.8-1.0 = sports car
            vehicle_type_map = {
                'heavy_truck': 0.15,
                'truck': 0.25,
                'bus': 0.35,
            ...
            vtype = args.vehicle_type.lower()
            if vtype in vehicle_type_map:
                vd_value = vehicle_type_map[vtype]
                print(f"  Vehicle type override: {args.vehicle_type} → dynamics={vd_value:.2f}")
            else:
                try:
                    vd_value = float(vtype)  # Allow numeric input 0-1
                except:
                    vd_value = 0.5  # Default
            vehicle_dynamics = torch.full((n_samples,), vd_value)
        else:
            vehicle_dynamics = torch.tensor([c['vehicle_dynamics'] for c in sampled])
    else:
        ...
        vehicle_dynamics = torch.rand(n_samples) * 0.6 + 0.2  # Mid-range default
    
    cond = torch.stack([
        avg_speeds.float(),
        durations.float() / 1000.0,
        max_speeds.float(),
        vehicle_dynamics.float(),  # Vehicle type/dynamics indicator
    ...
    print(f"    Vehicle dynamics: {vehicle_dynamics.min():.2f} - {vehicle_dynamics.max():.2f} (0=heavy truck, 1=sports car)")
    if args.correlated_noise > 0:
        print(f"    Correlated noise: std={args.correlated_noise}, corr_len={args.noise_corr_len}")
    
    trajectories = sampler.sample(
        cond, lengths, 
        smooth_kernel=args.smooth_kernel,
        correlated_noise_std=args.correlated_noise,
        noise_correlation_length=args.noise_corr_len
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--correlated_noise", type=float, default=0.0, help="Correlated noise std to add (0=disable)")
    parser.add_argument("--noise_corr_len", type=int, default=10, help="Correlation length for noise smoothing")
    parser.add_argument("--diffusion_steps", type=int, default=CONFIG["diffusion_steps"])
    parser.add_argument("--vehicle_type", type=str, default=None, 
                        help="Vehicle type: heavy_truck, truck, bus, suv, car, sedan, sports_car, or 0-1 value")
    parser.add_argument("--speed_boost", type=float, default=1.0,
                        help="Power for weighted speed sampling (1.0=uniform, 1.5=slight boost, 2.0=strong boost to high speeds)")
