...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --speed_boost) SPEED_BOOST="$2"; shift ;;
...
echo "Suffix: $SUFFIX"
echo "Speed Boost: $SPEED_BOOST"

OUTPUT_CSV="data/synthetic_trajectories${SUFFIX}.csv"

python diffusion_trajectory.py --generate --model_path "$MODEL_PATH" --cfg_scale "$CFG_SCALE" --n_samples "$N_SAMPLES" --output_file "$OUTPUT_CSV" --speed_boost "$SPEED_BOOST"
python evaluate_distribution.py --suffix "$SUFFIX" --synthetic "$OUTPUT_CSV"
