...
CONFIG = {
    ...
    # Diffusion parameters (more steps = smoother generation)
    "diffusion_steps": 200,
    "beta_start": 0.0001,
    "beta_end": 0.02,
    "schedule": "linear",
...
class CSDISampler:
    ...
    @torch.no_grad()
    def sample(self, cond, lengths, n_steps=None, boundary_ramp=15, smooth_kernel=7):
        """
        ...
        for t_idx in tqdm(steps, desc="Sampling"):
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, cond, mask in pbar:
            ...
            # Combined loss with smoothness weight
...
def generate(args):
    ...
    print(f"Diffusion steps: {CONFIG['diffusion_steps']}, Smoothing kernel: {args.smooth_kernel}")
    
    trajectories = sampler.sample(cond, lengths, smooth_kernel=args.smooth_kernel)
    
    # Denormalize: [-1, 1] -> [0, speed_limit]
    trajectories = (trajectories.cpu().numpy() + 1) / 2 * CONFIG["speed_limit"]
    trajectories = np.clip(trajectories, 0, CONFIG["speed_limit"])
